// fetch.mjs — genera data/today.json amb dades TIC del dataset ybgg-dgi6
...
const TIC = [
  'informàtic','informatic','tic','programari','software','aplicaci','xarxa','network','ciber','seguretat',
  'cloud','núvol','nuvol','dades','data','intel·ligència','intelligencia','ia','ai','machine','servidor','servidors',
  'hardware','microserveis','api','devops','plataforma','telecom','xarxes','m365','microsoft 365','sap','datacenter'
]
...
function isTIC(text){
  ...
  return TIC.some(k => t.includes(k.toLowerCase()))
}
...
function filterClient(items){
  ...
  return items
    ...
    .filter(it => {
      ...
      if(min && d && d < min) return false
      if(!isTIC(it.objecte_contracte)) return false
      return true
    })
...
async function run(){
  ...
  const where = `data_publicacio_anunci >= '${daysAgoISO(DAYS_BACK)}' AND (`+
    TIC.map(k=>`lower(objecte_contracte) like '%${k.toLowerCase()}%'`).join(' OR ')+`)`
  ...
  try{
    ...
    // Fallback: exportació + filtre al client
